...
3. 첫 도달 용량을 기준으로 상향 조정 옵션 최대 12개 생성
4. **현용법 용량을 첫 번째 옵션으로 추가** (중복 제거)
   - 예: 현재 용량이 345mg, 첫 도달 용량이 500mg이고 step이 10mg인 경우
   - 옵션: 345mg (현용법), 500mg, 510mg, 520mg, ..., 610mg (총 최대 13개)
...
- 옵션 생성: 현용법 용량 + 첫 도달 용량 기준으로 12개 옵션 생성 (API 호출 없음, 계산만)

**정렬 방식:**
- 현용법(1순위, 하이라이트) + 추천 12개 오름차순 정렬
...
2. 목표치에 도달하는 첫 번째 용량을 찾을 때까지 반복 (최대 20단계)
3. 첫 도달 용량을 기준으로 하향 조정 옵션 최대 12개 생성
4. **현용법 용량을 첫 번째 옵션으로 추가** (중복 제거)
   - 예: 현재 용량이 800mg, 첫 도달 용량이 400mg이고 step이 10mg인 경우
   - 옵션: 400mg, 390mg, 380mg, ..., 290mg, 800mg (현용법) (총 최대 13개)
